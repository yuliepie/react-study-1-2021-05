...
const StateAndUseState = () => {
    ...
    return (
        <div>
            ...
            <div onClick={onClickIncreaseBtn}>+</div> // state 조작 하는 콜백 bind
            1<div onClick={onClickDecreaseBtn}>-</div> // 2
        </div>
    );
};
...
class StateAndUseState extends React.Component {
    ...
    // @babel/transform-class-properties 문법으로 arrow function 으로
    // 표현가능
    onClickIncreaseBtn = (_) => {
        this.setState((prv) => prv + 1);
    };
...
> 이전의 함수 컴포넌트에서 사용 되었다 싶이, function components 에서 local
> state 접근에 사용
